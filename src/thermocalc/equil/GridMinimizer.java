 ...
 *
 *   1. MODEL-AGNOSTIC.
 *      This class calls exactly two methods on PhaseModelPort:
 ...
 *   2. Evaluate G via PhaseModelPort.evaluateG() for all phases at all
...
public class GridMinimizer {
    ...
    /** Mole-fraction floor applied to every grid point to keep G finite. */
    private static final double X_FLOOR = 1.0e-6;

    public GridMinimizer()            { this(20); }
    ...
    public EquilibriumState initialize(List<PhaseModelPort> candidates,
        ...
        return buildState(candidates, facet, xOverall, np);
    }
     ...
     * Each point is a double[nc] that sums to 1, every component >= X_FLOOR.
     */
    ...
    private double[] countsToY(int[] cnt, int nc) {
        ...
        double sum = 0;
        for (int i = 0; i < nc; i++) {
            y[i] = Math.max((double) cnt[i] / density, X_FLOOR);
            sum += y[i];
        }
    ...
    private EquilibriumState buildState(List<PhaseModelPort> candidates,
                                         FacetResult facet,
        ...
        // TODO: T and P should come from the initialize() caller
        double T = 1000; // Placeholder
        double P = 1e5;  // Placeholder
        return new EquilibriumState(T, P, xOverall, allPhases);
    }

    // =========================================================================
    //  Safe PhaseModelPort wrappers
    // =========================================================================

    /** Evaluate G, returning Double.MAX_VALUE on any exception. */
    private double safeG(PhaseModelPort m, double T, double[] y) {
    ...
    private double[] safeInitVars(PhaseModelPort m, double[] y) {
        try {
