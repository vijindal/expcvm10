...
public class PhaseChangeHandler {
    ...
    /** Bisection terminates when the axis interval is smaller than this. */
    private static final double BISECT_TOL       = 1e-6;
    /** Maximum bisection iterations. */
    private static final int    MAX_BISECT       = 50;
    /** Driving-force threshold for classifying a phase as "entering". */
    private static final double DRIVING_FORCE_TOL = 1e-4;
    /** Node de-duplication tolerance (axis distance). */
    private static final double NODE_MERGE_TOL   = 1e-5;
    ...
    public DiagramNode handle(DiagramLine line,
        ...
        List<PhaseModelPort> active = exclude(candidates, enteringPhase, null);
        ...
        DiagramNode existing = findNearbyNode(diagram, nodeAxes, NODE_MERGE_TOL);
        if (existing != null) {
    ...
    private String findEnteringPhase(EquilibriumResult eq) {
        for (EquilibriumResult.PhaseResult pr : eq.getMetastablePhases()) {
            if (pr.drivingForce > DRIVING_FORCE_TOL) return pr.phaseName;
        }
        return null;
    }
    ...
    private List<PhaseModelPort> exclude(List<PhaseModelPort> all,
