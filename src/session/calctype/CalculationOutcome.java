...
/**
 * Result of an {@link AssessingType} run. A sealed type rather than a thrown
 * exception so every caller (CLI/GUI/API) renders "not implemented yet" the
 * same way, without each having to catch a different exception type -- the
 * way {@code CalculationSession.calculatePhaseDiagram}'s
