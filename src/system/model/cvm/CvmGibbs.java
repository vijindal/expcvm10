...
public class CvmGibbs {
    ...
    /** Reference Gibbs energy evaluators per component (GHSER). */
    ...
    public double[] gradientDT(double[] u2vals, double T, double[] cec) {
        ...
        // ∂(∂G₀/∂x[m])/∂T = dGHSER_m/dT (numerical derivative)
        double h = 0.01;
        for (int m = 0; m < data.nComp; m++) {
            double dGhser_dT = (ghser[m].ghser(T + h) - ghser[m].ghser(T - h)) / (2.0 * h);
            gxt[ncf + m] += dGhser_dT;
        }
    ...
    public double g0(double[] u2vals, double T) {
        ...
        for (int i = 0; i < data.nComp; i++) {
            g += u2vals[data.ncf + i] * ghser[i].ghser(T);
        }
        return g;
    }
