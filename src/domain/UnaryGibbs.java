 ...
 * {@link TdbUnaryGibbs} instances (one per element) populated by parsing a
 ...
 * in Java — temperature range selection is handled inside each {@link TdbUnaryGibbs}.
 *
...
public class UnaryGibbs {
    ...
    private final Map<String, TdbUnaryGibbs> registry = new LinkedHashMap<>();
     ...
     *
     * <p>For each element name returned by {@link tdb#getElementNames()}, attempts
     * to construct a {@link TdbUnaryGibbs}. Elements with no G parameters (e.g. VA,
     * or elements whose data are defined as FUNCTION references that were not resolved)
    ...
    public void registerFromTdb(tdb database, String... elementFilter) {
        ...
        // Try to get element names from database
        java.util.List<String> elementNames = new java.util.ArrayList<>(database.getElementNames());

        // If database.getElementNames() returns empty, use a fallback list of common elements
        // This handles TDB files that don't populate element names but do have G parameters
        if (elementNames.isEmpty()) {
            elementNames = java.util.Arrays.asList(
                "AG", "AL", "AR", "AS", "AU", "B", "BA", "BE", "BI", "BK",
                "C", "CA", "CD", "CE", "CF", "CM", "CO", "CR", "CS", "CU",
                "DY", "ER", "ES", "EU", "FE", "FM", "FR", "GA", "GD", "GE",
                "H", "HE", "HF", "HG", "HO", "I", "IN", "IR", "K", "KR",
                "LA", "LI", "LU", "MD", "MG", "MN", "MO", "N", "NA", "NB",
                "ND", "NE", "NI", "NO", "NP", "O", "OS", "P", "PA", "PB",
        ...
        for (String elemName : elementNames) {
            ...
            try {
                registry.put(key, new TdbUnaryGibbs(elemName, database));
    ...
    private TdbUnaryGibbs lookup(String element) {
        TdbUnaryGibbs eg = registry.get(element.toUpperCase());
        if (eg == null) {
