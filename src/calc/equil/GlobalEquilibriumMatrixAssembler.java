...
public final class GlobalEquilibriumMatrixAssembler {
    ...
    /**
     * The genuine invariant-node system (Sundman Eq. 8/9): fixes TWO
     * phases' amounts at zero simultaneously and releases TWO
     * conditions (temperature AND one composition component) together,
     * rather than {@link #convertToFixedPhaseAmountSystem}/{@link
     * #convertToFixedPhaseAmountSystemReleasingT}'s single-phase/
     * single-condition release.
     *
     * <p><b>Why the single-release methods above cannot locate an
     * invariant</b>: for a binary system (nc=2) with p=3 stable phases
     * at the node (Eq. 8's own count for an isobaric binary invariant,
     * f=n+2-p-c=0 with T as the axis), the phase-equilibrium block is 3
     * rows ({@code sum_A M_A^k*lambda_A=G^k}, one per stable phase) but
     * only nc=2 of the n+np=5 columns (the lambda columns) are ever
     * nonzero in ANY phase-equilibrium row -- {@link #buildMatrix}
     * never puts a DeltaOmega coefficient in a phase-equilibrium row.
     * Three rows confined to two columns are linearly dependent by
     * construction, so the assembled 5x5 matrix is EXACTLY singular
     * regardless of the phases' compositions or the quality of any
     * initial guess -- confirmed directly this session (a Halton-best
     * driving-force seed for the new phase still produced "Matrix is
     * singular" on iteration 0). Releasing only one extra scalar (T or
     * one composition target) adds only one new nonzero entry to this
     * block (in the fixed phase's own row) and cannot repair a
     * rank-2-in-3-rows deficiency.
     *
     * <p><b>The fix, matching Eq. 9 exactly</b>: "Two of the n+1 phases
     * stable at the invariant must have zero amount... We can use the
     * remaining n-1 phases and the n-1 conditions to formulate a system
     * of linear equations." For binary (n=2): fix 2 of the 3 phases at
     * zero (removing 2 DeltaOmega columns, leaving only 1 -- for the
     * remaining, genuinely-adjustable phase), and correspondingly
     * release 2 conditions (T and one composition component) to keep
     * the system square: {@code nc + np - 2} ordinary unknowns (lambda
     * x2, DeltaOmega x1) plus 2 released-condition unknowns (DeltaT,
     * Delta-target-composition) = 5 again, but now the phase-equilibrium
     * block's rank deficiency is gone -- each of the two fixed phases'
     * own rows now ALSO carries a nonzero DeltaT entry ({@code
     * -phaseData[k].dG_dT}), breaking the "only 2 nonzero columns across
     * 3 rows" degeneracy that made the single-release systems singular.
     *
     * <p>Column layout of the two freed columns:
     * <ul>
     *   <li>{@code fixedColumnA} (was DeltaOmega_fixedSlotIndexA) --
     *       becomes the DeltaT column: {@code -phaseData[k].dG_dT} in
     *       EVERY stable phase's own phase-equilibrium row {@code k}
     *       (not just the fixed ones -- T is a global condition every
     *       phase responds to), plus {@code
     *       sum_k omega_k*phaseData[k].dM_dT[A]} in every mass-balance
     *       row {@code np+A}, exactly as {@link
     *       #convertToFixedPhaseAmountSystemReleasingT} already
     *       derives.</li>
     *   <li>{@code fixedColumnB} (was DeltaOmega_fixedSlotIndexB) --
     *       becomes the released-composition column: {@code -1} in mass
     *       balance row {@code np+releasedComponentIndex}, 0 elsewhere,
     *       exactly as {@link #convertToFixedPhaseAmountSystem}
     *       derives.</li>
     * </ul>
     *
     * @param fixedSlotIndexA one of the two phases fixed at zero amount
     * @param fixedSlotIndexB the other phase fixed at zero amount
     * @param releasedComponentIndex composition component released
     *                               alongside T
     * @return a NEW (matrix, rhs) pair -- the inputs are not mutated
     */
    public static Result convertToInvariantNodeSystem(
            double[][] matrix,
            double[] rhs,
            PhaseEquilData[] phaseData,
            double[] phaseAmounts,
        ...
        int fixedColumnA = nc + fixedSlotIndexA;
        int fixedColumnB = nc + fixedSlotIndexB;

        for (int row = 0; row < n; row++) {
            A[row][fixedColumnA] = 0.0;
            A[row][fixedColumnB] = 0.0;
        }

        // fixedColumnA -> DeltaT.
        for (int k = 0; k < np; k++) {
            A[k][fixedColumnA] = -phaseData[k].dG_dT;
        }
        for (int Aidx = 0; Aidx < nc; Aidx++) {
            double coeff = 0.0;
            for (int k = 0; k < np; k++) {
            ...
            A[np + Aidx][fixedColumnA] = coeff;
        }

        // fixedColumnB -> Delta(released composition target).
        A[np + releasedComponentIndex][fixedColumnB] = -1.0;
