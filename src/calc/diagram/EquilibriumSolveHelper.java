...
final class EquilibriumSolveHelper {
    ...
    /**
     * Calls {@link EquilibriumSolverV2#solveBoundary} (Sundman Algorithm
     * C2 -- the exact ZPF boundary solve, see that method's own detailed
     * javadoc for the mechanism), returning {@code null} instead of
     * throwing if the boundary solve fails to converge -- used only by
     * {@link MapTracer} at a detected stable-phase-set change, in place
     * of {@link StepTracer#trace}'s black-box bisection.
     */
    static EquilibriumSolverV2.BoundarySolveResult solveBoundaryOrNull(
            double t,
            double p,
            double[] comp,
            List<GibbsEnergyModel> candidates,
        ...
        try {
            ...
            return new EquilibriumSolverV2().solveBoundary(
                    t, p, comp, candidates, seed,
                    fixedPhaseName, fixedAmount, releasedComponentIndex);
