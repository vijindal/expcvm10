...
above (CalculationSession) and the layer it queries every iteration
(Thermodynamic System Layer / GibbsEnergyModel), not just the solver's
own internals in isolation.
...
Traced directly from the actual code:
  session.CalculationSession.calculateEquilibrium(T, P, compOverAll):
      currentEquilibriumResult = new EquilibriumSolverV2()
                                      .solve(T, P, compOverAll, currentSystem().phaseModels())
...
# ── Title ─────────────────────────────────────────────────────────
ax.text(LOOP_CX, FIG_H - 0.4, "Calculation Layer -- Hillert/Sundman Equilibrium Iteration",
...
# session -> solve() in, and result back out (two separate vertical arrows)
arrow(LOOP_CX - 1.0, b_session['bot'], LOOP_CX - 1.0, loop_top_y, COL_SESSION, lw=2.6)
label(LOOP_CX - 1.9, (b_session['bot'] + loop_top_y) / 2,
      "solve(T, P,\ncompOverAll,\ncandidates)", color=COL_SESSION, fs=8.5)
arrow(LOOP_CX + 1.0, loop_top_y, LOOP_CX + 1.0, b_session['bot'], COL_CALC, lw=2.6)
label(LOOP_CX + 2.0, (b_session['bot'] + loop_top_y) / 2,
      "EquilibriumResult\n(stored, not\nreturned)", color=COL_CALC, fs=8.5)
...
# ── GridMinimizer note, feeding STEP 1's "start values" (dashed: not
#    yet wired) ──────────────────────────────────────────────────────
grid_cy = row_y[0] + top_h + 1.1
b_grid = box(fb_x - 0.35, grid_cy, 3.0, 1.05,
             "GridMinimizer",
             "exists (calc/equil) --\nnot yet wired into\nthese start values",
             COL_START, title_fs=10, sub_fs=8, dashed=True)
elbow(b_grid['right'], b_grid['bot'], boxes[(0,0)]['left'] + 0.05, row_y[0] + 0.35,
      "#999", lw=1.6, dashed=True, angleA=0, angleB=90)
label(b_grid['cx'], b_grid['top'] + 0.35, "Sundman 2015 Fig. 3-4:\nstart-value grid search",
      color="#777", fs=8, style="italic")
...
legend = [
    ...
    Line2D([0], [0], marker='s', color='w', markerfacecolor=COL_START, markersize=14, label='Start values / initialization'),
...
all_y = [FIG_H - 0.15] + [b['bot'] for b in boxes.values()] + [b_sys['bot'], b_result['bot'], b_grid['bot']]
all_x = [b_sys['right'] + 0.3, b_grid['left'] - 0.3]
