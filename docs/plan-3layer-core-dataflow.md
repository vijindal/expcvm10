...
> future UI (GUI, CLI, API), not just the Swing GUI. A second correction,
> same day: results flow INTO `CalculationSession` (not straight back to
> the caller), UI↔session is genuinely two-way, and System↔Calculation is a
...
via shared code). See revision 3 below for the design that adds that.

## Target data flow, revision 3 (corrected topology — current design)

![Target core data flow](dataflow_target.png)

*(Generated by `docs/gen_target_dataflow_figure.py`; re-run and re-embed
after any structural change to keep this in sync.)*

This corrects two things from the first `CalculationSession` sketch
(revision 2, kept below for history), per further discussion with the user
on 2026-09-09:

1. **Results flow into `CalculationSession`, not directly back to the
   caller.** The Calculation Layer hands its result to the session, which
   holds it (alongside the current system) as `currentResult`; the UI reads
   the result *from the session* rather than getting it handed back by the
   `calculate(...)` call itself. This is why `calculateEquilibrium(...)`/
   `calculatePhaseDiagram(...)` in the sketch below are `void` — they store,
   they don't return.
2. **UI ↔ `CalculationSession` is genuinely two-way**, drawn as one
   bidirectional arrow: the UI sends model/calculation details down, and
   independently reads results/status back up, rather than one call
   producing one synchronous answer. `CalculationSession` is the **only**
   thing the UI ever talks to — it never reaches into `ThermodynamicSystem`
   or a solver directly, in either direction.
3. **System Layer ↔ Calculation Layer is a real, repeated, two-way loop**
   within a single `calculate(...)` call, not a one-time handoff of
   `GibbsEnergyModel[]`. This is just making Sundman's Algorithm A visible
   in the diagram (see Context above): every Newton iteration is a fresh
   `(T, y)` query answered with fresh `G, dG/dy, d2G/dy2` — the System Layer
   holds no state between calls, so this exchange repeats as many times as
   the solver needs before `CalculationSession` ever sees a finished result.

`CalculationSession` (purple) is still the single choke point every caller
(GUI, future CLI, future API) goes through, and `setModel(...)` still
rebuilds only when the model-details key changes. `EquilibriumUseCase`/
...
`CalculationSession` rather than `ThermodynamicSystem.build()` or a solver
directly.

<details>
<summary>Target data flow, revision 2 (superseded — results returned directly, no repeated loop shown)</summary>
...
- `CalculationSession` does no thermodynamics or numerics itself. It holds
  state — the currently-built `ThermodynamicSystem`, if any, **and the
  most recent calculation result** — and forwards: building the system to
  `ThermodynamicSystem.build(...)` (System Layer), running a calculation to
  the appropriate Calculation Layer entry point (`EquilibriumSolver`,
  `DiagramTracer`, etc.), passing along the held system.
- **Results flow back to `CalculationSession`, not directly to the UI**
  (design correction, 2026-09-09, from further discussion with the user).
  The Calculation Layer hands its result to `CalculationSession`, which
  holds it; the UI then reads the result *from the session*
  (`currentResult()`/similar), rather than a calculation call handing the
  result straight back up the stack. This makes `CalculationSession` the
  UI's **single point of contact both ways** — the UI never talks to
  `ThermodynamicSystem`, `EquilibriumSolver`, or `DiagramTracer` directly,
  in either direction.
- **UI ↔ `CalculationSession` is two-way**, not one-shot request/response:
  the UI can query session state (is a model loaded? is a calculation
  still running? what was the last result?) independently of triggering a
  new calculation. This matters once calculations run asynchronously
  (e.g. a long phase-diagram scan) — the UI polls or is notified rather
  than blocking on a single call that returns the answer.
- **System Layer ↔ Calculation Layer is two-way and happens *repeatedly*
  within a single calculation**, not once. This isn't new physics — it's
  making explicit what Sundman's Algorithm A already does (see the Context
  section above): every Newton iteration, the Calculation Layer asks the
  System Layer to evaluate `siteEnergy`/`siteGradient`/`siteHessian` at a
  *new* `(T, y)`, because the solver needs fresh values at each updated
  trial point, not just once at the start. The System Layer has no memory
  of previous calls — it's a pure function of whatever `(T, y)` it's asked
  about at that moment — so this loop runs as many times as the solver
  needs, entirely within one `CalculationSession.calculate*(...)` call.
...
public final class CalculationSession {
    ...
    private Object currentResult;              // null until a calculate*() has completed;
                                                // EquilibriumResult, PhaseDiagram, etc.
    ...
    public void setModel(String tdbFilePath, List<String> elements, List<String> phases)
        ...
        this.currentResult = null;   // old result no longer valid for a new system
    }
     ...
     * held system, storing the result on the session (does NOT return it
     * directly -- the caller reads it back via currentResult()/a typed
     * accessor). Requires setModel() to have been called first. Internally,
     * EquilibriumSolver.solve(...) calls back into the held
     * GibbsEnergyModel instances many times (once per Newton iteration)
     ...
     * storing the result on the session. Same many-calls-per-one-return
     * pattern as calculateEquilibrium, but with far more System Layer
     * calls (once per node/line-step, each itself running many solver
     * iterations).
     */
    public void calculatePhaseDiagram(AxisConfig[] axes, double[] startAxes,
                                       double fixedT, double fixedP, double[] comp) {
        this.currentResult = new DiagramTracer().calculate(
                currentSystem().phaseModels(), axes, startAxes, fixedT, fixedP, comp);
    }

    /** The most recent calculation result, or null if none has completed yet. */
    public Object currentResult() { return currentResult; }
}
```

The `Object currentResult` typing above is a placeholder for the sketch,
not a real design choice — see the open question below on how to type
this properly (a sealed/typed result holder, or separate
`currentEquilibriumResult()`/`currentPhaseDiagram()` accessors) before
implementation.
...
**Also still open:** how `currentResult()` should be typed so callers get a
real result type back (`EquilibriumResult` vs `PhaseDiagram`) without an
unchecked cast — a sealed interface, one accessor per calculation kind, or
a generic `Result<T>` wrapper are all plausible; not decided yet.
...
session.calculateEquilibrium(2000, 101325, new double[]{0.5, 0.5});
EquilibriumResult r1 = (EquilibriumResult) session.currentResult();
// ... user changes T only, model unchanged -- setModel() below is a no-op:
session.setModel("data/VZR-re2.TDB", List.of("V", "ZR"), List.of("LIQUID", "BCC_A2"));
session.calculateEquilibrium(1800, 101325, new double[]{0.5, 0.5});
EquilibriumResult r2 = (EquilibriumResult) session.currentResult();
```
